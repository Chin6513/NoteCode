...
上述代码是什么意思呢？首先，将结构对象team作为第一个参数传递给了accumulate0。
这意味着在函数accumulate(0中，target指向的是team。函数accumulate0修改team,再返回指向它的引用。
注意到返回语句如下：
return target;
...
但返回类型为引用，这意味着返回的是最初传递给accumulateO的team对象。
接下来，将accumulate(O的返回值作为参数传递给了display(),这意味着将team传递给了display()。
displayO的参数为引用，这意味着函数 display()中的ft指向的是 team,因此将显示 team 的内容。所以，下述代码：
display (accumulate (team, two));
与下面的代码等效：
